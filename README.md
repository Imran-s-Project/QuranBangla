...
- **স্মার্ট অফলাইন/PWA ইঞ্জিন** (`js/smart-storage.js`) — ডিভাইসের স্টোরেজ কোটার কাছাকাছি পৌঁছালে সবচেয়ে কম-ব্যবহৃত (LRU) অফলাইন সূরাগুলো নিজে থেকেই সরিয়ে জায়গা খালি করে; নতুন ভার্সন এলে স্বয়ংক্রিয়ভাবে রিফ্রেশ হয়ে সবসময় একই ভার্সনের ফাইল নিয়ে চলে; ইন্টারনেট চলে গেলে ক্লাউড-সিঙ্ক (স্ট্যাটস) হারিয়ে না গিয়ে "pending" থাকে ও সংযোগ ফিরলেই স্বয়ংক্রিয়ভাবে আবার পাঠানো হয়
...
└── js/
    ...
    ├── smart-storage.js      # কোটা-সচেতন LRU অডিও eviction (অফলাইন ক্যাশ যেন কখনো ব্রাউজারের হাতে এলোমেলোভাবে না মোছে)
...
## এক্সাম অ্যাডমিন প্যানেল (`admin.html`)

সাইটে এক্সামের লিংক যোগ/পরিবর্তন/মোছার জন্য আলাদা একটি অ্যাডমিন-অনলি পেজ আছে — `admin.html`। সাধারণ ইউজাররা মূল অ্যাপের হ্যামবার্গার মেনু থেকে "পরীক্ষা দিন" চেপে শুধু তালিকাটা রিয়েল-টাইমে দেখতে ও লিংকে ক্লিক করতে পারবে (`js/exam.js`); অ্যাডমিন প্যানেলের কোনো অংশ মূল অ্যাপে দেখানো হয় না।

**একবারের সেটআপ (নিজেকে অ্যাডমিন বানানো):**
1. Firestore-এ নতুন rules ডিপ্লয় করুন (`firestore.rules` — এতে `exams` কালেকশনের rule যোগ করা আছে)।
2. মূল সাইটে (`index.html`) সাধারণ ইউজারের মতোই একটি অ্যাকাউন্ট দিয়ে সাইন-আপ/লগইন করুন (ইমেইল+পাসওয়ার্ড)।
3. Firebase Console → Firestore Database → `admins` কালেকশনে ঢুকুন (না থাকলে নতুন তৈরি করুন)।
4. আপনার অ্যাকাউন্টের UID (Authentication ট্যাব থেকে কপি করা যাবে) দিয়ে একটি ডকুমেন্ট বানান, এবং তাতে একটি ফিল্ড যোগ করুন: `isAdmin` (boolean) = `true`।
5. এখন `admin.html`-এ গিয়ে সেই একই ইমেইল/পাসওয়ার্ড দিয়ে লগইন করলেই অ্যাডমিন প্যানেল খুলবে।

`admins/{uid}` ডকুমেন্ট Firebase Console থেকে ছাড়া আর কোথাও থেকে (এমনকি অ্যাডমিন প্যানেল থেকেও) লেখা যায় না (`firestore.rules` → `allow write: if false`) — এটা ইচ্ছাকৃতভাবে রাখা হয়েছে, যাতে কেউ কখনো নিজেকে নিজে অ্যাডমিন বানিয়ে নিতে না পারে।

**পরীক্ষার "প্রকাশের সময়" (লাইভ কাউন্টডাউন):** পরীক্ষা যোগ/এডিট করার সময় ঐচ্ছিক `প্রকাশের সময়` ফিল্ডে একটি ভবিষ্যৎ তারিখ-সময় দিলে, সেই সময় না আসা পর্যন্ত ইউজার সেকশনে পরীক্ষাটি ক্লিকযোগ্য কার্ড হিসেবে না দেখিয়ে একটি "শীঘ্রই আসছে" কার্ডে দিন:ঘণ্টা:মিনিট:সেকেন্ড লাইভ কাউন্টডাউন দেখানো হয় (`js/exam.js`)। সময় হয়ে গেলে কোনো রিফ্রেশ বা নতুন Firestore আপডেট ছাড়াই কার্ডটি নিজে থেকে স্বাভাবিক ক্লিকযোগ্য কার্ডে পরিণত হয়ে যায়। ফিল্ডটি খালি রাখলে পরীক্ষাটি যোগ করার সাথে সাথেই সবার কাছে প্রকাশিত থাকে। এটি কাজ করতে `firestore.rules`-এ `exams` কালেকশনের rule-এ `publishAt` ফিল্ড (Timestamp) যোগ করা আছে — Firebase Console-এ rules রি-ডিপ্লয় করলেই যথেষ্ট।
